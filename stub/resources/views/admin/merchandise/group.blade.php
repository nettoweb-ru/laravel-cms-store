<x-cms::layout.admin :title="$title" :chain="$chain" :header="$header">
    <x-cms::tabs id="group_tab"
        ...
        @if ($object->id)
            <x-slot name="tab2">
                <x-cms::list
                    :url="route('admin.group.list', ['parent' => $object->id], false)"
                    id="store-group-{{ $object->id }}"
                    :columns="[
            ...
            <x-slot name="tab3">
                <x-cms::list
                    :url="route('admin.merchandise.list', ['parent' => $object->id], false)"
                    id="store-merchandise-{{ $object->id }}"
                    :columns="[
                        'id' => __('cms::main.attr_id'),
                        'sort' => __('cms::main.attr_sort'),
                        'name' => __('cms::main.attr_name'),
