...
{
    private const TABLE = 'cms_store__prices__lang';
    private const COLUMNS = [
        'object_id' => 'cms_store__prices',
        'lang_id' => 'cms__lang',
    ];
    ...
    public function up(): void
    {
        ...
        $default = DB::table('cms_store__prices')->select('id')->where('is_default', '1')->get()->get(0);
        $language = DB::table('cms__lang')->select('id')->where('is_default', '1')->get()->get(0);

        DB::table(self::TABLE)->insert([
            ['object_id' => $default->id, 'lang_id' => $language->id, 'name' => 'Розничная цена'],
        ]);
    }
